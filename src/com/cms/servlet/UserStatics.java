 ...
 */
public class UserStatics extends HttpServlet{
    /**
	 * 
	 */
	private static final long serialVersionUID = 2016049695251408341L;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
        ...
        try{
            ...
            Vector trafic = Log.getUserStatics(Integer.parseInt(request.getParameter("idUser")), request.getParameter("startDate"), request.getParameter("endDate"), ((Session)request.getSession().getAttribute("sessionApp")).getSite().getIdSite());
        	...
        	while(dateS.getTimeInMillis() <= dateE.getTimeInMillis()){
        	    ...
        	    if(vectorIndex < trafic.size()){
        	        ...
        	        if(row.get("DATE").toString().equals(dateS.get(Calendar.YEAR) + "-" + (dateS.get(Calendar.MONTH) + 1 < 10? "0" : "") + (dateS.get(Calendar.MONTH) + 1) + "-" + (dateS.get(Calendar.DATE) < 10? "0" : "") + dateS.get(Calendar.DATE))){
        	            seriesY1 += row.get("TOTAL_ACCESS") + sep;
